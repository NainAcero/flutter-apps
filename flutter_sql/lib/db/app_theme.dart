...
class MyAppTheme extends ChangeNotifier {
  ...
  Future<void> init() async {
    this._darkEnabled = (await this._store.record('DARK_ENABLED').get(this._db!)) ?? false;
  }

  change(bool darkEnabled) async {
    this._darkEnabled = darkEnabled;
    final dataSaved = await this._store.record('DARK_ENABLED').put(this._db!, this._darkEnabled);
