...
import 'package:flutter_clean_arquitect_whatsapp/data/model/my_chat_model.dart';
import 'package:flutter_clean_arquitect_whatsapp/data/model/text_message_model.dart';
import 'package:flutter_clean_arquitect_whatsapp/data/model/user_model.dart';
...
class FirebaseRemoteDataSourceImpl implements FirebaseRemoteDataSource {
  ...
  Future<void> createOneToOneChatChannel(String uid, String otherUid) async{
    ...
    userCollectionRef.doc(uid).collection('engagedChatChannel').doc(otherUid).get().then((chatChannelDoc) {
      ...
      //currentUser
      userCollectionRef.doc(uid).collection("engagedChatChannel").doc(otherUid).set(channelMap);

      //OtherUser
      userCollectionRef.doc(otherUid).collection("engagedChatChannel").doc(uid).set(channelMap);
  ...
  Stream<List<UserEntity>> getAllUsers() {
    ...
    return userCollectionRef.snapshots().map((querySnapshot) {
      return querySnapshot.docs.map((docQuerySnapshot) => UserModel.fromSnapshot(docQuerySnapshot)).toList();
    });
  }
  ...
  Stream<List<TextMessageEntity>> getMessages(String channelId) {
    final messagesRef = fireStore.collection("myChatChannel").doc(channelId).collection('messages');

    return messagesRef.orderBy('time').snapshots().map((querySnap) => 
        querySnap.docs.map((doc) => TextMessageModel.fromSnapShot(doc)).toList());
  }
  ...
  Stream<List<MyChatEntity>> getMyChat(String uid) {
    ...
    return myChatRef.orderBy('time', descending: true).snapshots().map((querySnap) 
      => querySnap.docs.map((doc) => MyChatModel.fromSnapShot(doc)).toList());
  }
  ...
  Future<String> getOneToOneSingleUserChannelId(String uid, String otherUid) {
    ...
    return userCollectionRef.doc(uid).collection('engagedChatChannel').doc(otherUid).get().then((engagedChatChannel) {
  ...
  Future<void> sendTextMessage(TextMessageEntity textMessageEntity, String channelId) async {
    ...
    final messageRef = fireStore.collection('myChatChannel').doc(channelId).collection('messages');
  ...
  Future<void> verifyPhoneNumber(String phoneNumber) async {
    ...
    final PhoneVerificationFailed phoneVerificationFailed = (FirebaseAuthException firebaseAuthException){
